...
// The integer upscale the stage is drawn at. Fixed, in every window: 320×224 at
// ×3 is the 960×672 the game shipped at, and that is the size it stays. A
// bigger window gets more of index.html's near-black ground around the picture,
// never a bigger picture — the frame grows, the arcade screen inside it does
// not. Whole number, because at 2.5× a logical pixel lands on two and a half
...
// antialiased glyph edges scaled like a sprite read as blur. Read when the Text
// is created; main.ts re-reads it on every Text on the stage when browser zoom
// moves the device ratio under them.
